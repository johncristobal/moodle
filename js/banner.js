...
$(document).ready(function()
{
	...
	*/
	...
	initAccordions();
        ...
        $("#foto").change(function(elemento){
            //del elemento seleccionadosubo y actualizo foto del respectivo indice
            var file = elemento.target.files[0];
            var imagefile = file.type;
            var match= ["image/jpeg","image/png","image/jpg"];
            if(!((imagefile==match[0]) || (imagefile==match[1]) || (imagefile==match[2])))
            {
                $('#previewing').attr('src','<?php echo base_url();?>banners/templateBanner.png');
        ...
        $("#form").submit(function(e){
            ...
            function(isConfirm){
        	if(isConfirm){
                    //borra anuncio...solo borra y ya
                    //el orden no cambia, solo suma 1
                    console.log("guardando banner");
                    $.ajax({
                        type:'POST',
                        url: urlApi+"/admin/saveBanner",
                        data:formData,
                        cache:false,
                        contentType: false,
                        processData: false,
                        success:function(data){
                            console.log("success");
                            window.location.href = urlApi+"/admin/banners";
                        },
                        error: function(data){
                            console.log("error");
                            swal({   
        ...
        function deleteBanner(id){
            swal({
                title: "¿Seguro que deseas borrar este anuncio?",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#ffae01",
                confirmButtonText: "Continuar",
